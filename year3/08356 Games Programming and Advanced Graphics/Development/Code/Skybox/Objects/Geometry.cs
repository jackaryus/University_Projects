...
namespace OpenGL_Game.Objects
{
    public class Geometry
    {
        ...
        List<VertexPositionNormalTexture> vertices2 = new List<VertexPositionNormalTexture>();
        ...
        public void LoadObject(string filename)
        {
            ...
            try
            {
                ...
                while (!sr.EndOfStream)
                {
                    ...
                    vertices.Add(new VertexPositionTexture(new Vector3(float.Parse(values[0]), float.Parse(values[1]), float.Parse(values[2])), new Vector2(float.Parse(values[3]), float.Parse(values[4]))));
                }
            }
            catch (Exception e)
            {
